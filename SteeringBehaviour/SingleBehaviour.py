...
def GetHidingPosition(player_position, obstacle_position):
...
class SingleBehaviour:
    ...
    def hide(self, player_position):
        ...
        if Found_Hideout == True:
            move_vector = [c * self.agent.speed for c in normalize(sub(GetHidingPosition(player_position, best_hiding_spot), self.agent.pos))]
            Found_Hideout = False
